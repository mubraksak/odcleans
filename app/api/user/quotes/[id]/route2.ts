...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { action, scheduledDate, suggested_price, user_notes, additional_services } = await request.json()
    ...
    if (action === "accept") {
      ...
      await query(
        "UPDATE quote_requests SET suggested_price = ?, user_notes = ?,  status = 'counter offer' WHERE id = ?",
        [suggested_price, user_notes, quoteId]
      )
