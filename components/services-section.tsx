...
// Define service details mapping
const serviceDetailsMap: Record<string, string[]> = {
  "Residential Cleaning": [
    "Standard Cleaning",
    "Deep House Cleaning",
    "Regular Maintenace Cleaning",
  ...
  // Add fallback for any other service names
  "default": [
...
// Get the appropriate service details based on service name
const getServiceDetails = (serviceName: string) => {
  // Check if service name contains keywords to map to the right category
  if (serviceName.toLowerCase().includes("residential") || 
      serviceName.toLowerCase().includes("home") ||
      serviceName === "Standard Cleaning" ||
      serviceName === "Deep Cleaning") {
    return serviceDetailsMap["Residential Cleaning"];
  } else if (serviceName.toLowerCase().includes("commercial") || 
            serviceName.toLowerCase().includes("office") ||
            serviceName.toLowerCase().includes("business")) {
    return serviceDetailsMap["Commercial Cleaning"];
  }
  
  // Return the specific details if there's an exact match
  return serviceDetailsMap[serviceName] || serviceDetailsMap["default"];
}
                    ...
                    <ul className="space-y-3">
                      {details.map((detail, index) => (
                        <li key={index} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 hover:bg-accent/10 transition-all group">
                          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-circle-check-big w-5 h-5 text-secondary flex-shrink-0"><path d="M21.801 10A10 10 0 1 1 17 3.335"></path><path d="m9 11 3 3L22 4"></path></svg>
